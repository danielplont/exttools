	...
	public void receivePacket(Packet arg0) {
		if (arg0 instanceof TCPPacket) {
			...
			if (packet.data.length == 15 && packet.data[0] == 0x0d && packet.data[1] == 0) {
				...
				byte key[] = { 0x13, 0x00, 0x00, 0x00, 0x08, 0x00, 0x00, 0x00, 0x06, 0x00, 0x00, 0x00, (byte) 0xB4,
								0x00, 0x00, 0x00, 0x1B, 0x00, 0x00, 0x00, 0x0F, 0x00, 0x00, 0x00, 0x33, 0x00, 0x00,
				...
				send = new MapleAESOFB(key, ivsend, version);
				recv = new MapleAESOFB(key, ivrecv, (short) (0xFFFF - version));
		...
		if (crypto.checkPacket(header)) {
			...
			try {
				if (dis.available() >= packetLen) {
					...
					dis.readFully(ddata);
					// System.out.println(HexTool.toString(crypto.getIv()));
					// System.out.println(HexTool.toString(header));
					byte unc[] = new byte[ddata.length];
					System.arraycopy(ddata, 0, unc, 0, ddata.length);
					// System.out.println("undecrypted: " + HexTool.toString(ddata));
					crypto.crypt(ddata);
					// System.out.println("aes decrypted: " + HexTool.toString(ddata));
					MapleCustomEncryption.decryptData(ddata);
					String op;
					if (send) {
						op = lookupRecv(readFirstShort(ddata));
					} else {
						op = lookupSend(readFirstShort(ddata));
					}
					String start = send ? "Sent " : "Received ";
					System.out.println(start + op + " (" + packet.sec + "." + packet.usec + "): " + " (" + packetLen + ") ");
					System.out.println(HexTool.toString(ddata));
					System.out.println(HexTool.toStringFromAscii(ddata));
					//assert packetLen < 4000;
					return packetLen + 4;
				}
			} catch (IOException e) {
				e.printStackTrace();
