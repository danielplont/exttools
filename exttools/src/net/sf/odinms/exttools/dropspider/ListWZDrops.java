...
public class ListWZDrops {
	private final static String STRING_WZ_NAME = "JMSString.wz";
	...
	public void compareToMonsterbook() {
		...
		for (MapleData data : monsterBook) {
			...
			for (MapleData drop : data.getChildByPath("reward")) {
				drops.add(MapleDataTool.getInt(drop));
			}
			monsterBookDrops.put(monsterId, drops);
		}
						...
						if (!dbDrops.contains(itemDrop)) {
							...
							if (itemName != null) {
								...
								if (itemClass.getChance() >= 0) {
									...
									System.out.println("# Adding missing drop " + itemDrop + " (" + itemName + ") " +
									...
									if (!monster.isBoss() && !shouldUpdate(itemClass) && oldChance != null &&
										...
										System.out.println("# WARNING Adding with a different chance than the old without updating");
									}
									if (monster.isBoss() && !bosses.contains(monster.getId())) {
										bosses.add(monster.getId());
									}
									System.out.println("INSERT INTO monsterdrops (monsterid,itemid,chance) VALUES (" +
										monster.getId() + ", " + itemDrop + ", " + newChance + ");");
									numAdd++;
								} else {
									System.out.println("# Skipping  drop " + itemDrop + " (" + itemName + ") " +
										itemClass + " (chance = -1)");
								}
							} else {
								System.out.println("# Skipping Item " + itemDrop + " not in gms string.wz");
							}
						}
					}
				} else {
					System.out.println("# Monster " + monsterId + " not in GMS data");
				}
			}
		}
		System.out.println();
			...
			if (itemClass != null) {
				if (shouldUpdate(itemClass)) {
					if (dropChance.getValue() != itemClass.getChance()) {
						System.out.println("# Updating chance of " + itemId + "(" + mii.getName(itemId) + ") from " +
							dropChance.getValue() + " to " + itemClass.getChance() + " (" + itemClass + ")");
						System.out.println("UPDATE monsterdrops SET chance = " + itemClass.getChance() +
							" WHERE itemid = " + itemId + notABoss + ";");
						numUpdate++;
					} else if (dropChance.getValue() == -1) {
						System.out.println("# Not updating chance of " + itemId + "(" + mii.getName(itemId) +
							") since it's old chance is disamgious");
					}
				}
			} else {
				System.out.println("# Unclassified item: " + itemId + " (" + mii.getName(itemId) + ") chance: " +
					dropChance.getValue());
			}
		}
		System.out.println("# Totally adding " + numAdd + " drops and updating " + numUpdate + " dropchances");
	}
